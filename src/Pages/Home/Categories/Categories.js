...
const Categories = () => {
    const { data: categories = [] } = useQuery({
        queryKey: ['categories'],
        queryFn: async () => {
            const res = await fetch('https://cell-swap-server.vercel.app/categories');
    ...
    return (
        <div className='my-28 mx-auto container'>
            <h2 className="text-4xl font-bold text-center mb-6">Mobile Phones Categories</h2>
            <div className='grid lg:grid-cols-3 md:grid-cols-3 grid-cols-1 space-y-3'>
                {
                    categories.map(c =>
                        <div key={c._id} className='mx-auto p-8 rounded-md cursor-pointer shadow-md border-black'>
